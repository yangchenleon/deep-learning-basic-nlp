...
if __name__ == '__main__':
    ...
    rnn_layer = nn.LSTM(num_inputs, num_hiddens, num_layers, bidirectional=True)
    ...
    for epoch in range(num_epochs):
        ...
        data_iter = train_iter if (epoch + 1) % 5 != 0 else tqdm(train_iter, desc=f"Processing {epoch + 1:02}", ncols=100)
    ...
    torch.save(net.state_dict(), 'models/bi_rnn.pth')

    # predict (which is a bad application for bi-rnn)
    model = net
    model.load_state_dict(torch.load('./models/bi_rnn.pth', map_location=devices[0]))
